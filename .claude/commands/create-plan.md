...
1. **Read core reference files:**
   - The active constitution (`CLAUDE.md` or `AGENTS.md`) — workspace overview, structure, and conventions
...
2. **Explore relevant areas:**
   ...
   - If creating a skill: read existing canonical skills in `.claude/skills/` and plan the mirrored `.agents/skills/` copy
...
4. **Classify what you're building:**
   - Is this a Skill (available in both harnesses) or a Claude Code command shortcut?
   - Canonical skills live in `.claude/skills/{name}/SKILL.md` and are mirrored to `.agents/skills/` with `scripts/sync_harness_skills.py`.
   - Claude Code command shortcuts live in `.claude/commands/{name}.md`; a universal workflow also needs a matching skill.
...
3. Recommended next step: "Implement `plans/{filename}` when ready." In Claude Code, `/implement plans/{filename}` is an optional shortcut.
