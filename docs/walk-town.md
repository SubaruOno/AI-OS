...
- 「今日のこと」を閉じたボタンには、その日の次の行動を表示する。かなえた手紙→届いた包み→町名入力→持ち帰り候補→建てられる事業→渡せるおてつだい→手紙の順。包みと事業はボタンから直接それぞれの画面へ移し、そのほかは今日のパネルを開く。パネルは画面高の58%まで広がり、中の一覧に残りの高さを割り当てる。
...
- 追加確認: `./scripts/check.sh`、`git diff --check`、`npx expo lint` 通過。iOSシミュレーターで更新後のホームと「今日のこと」パネルを開き、持ち帰り候補・おてつだい・手紙まで表示されることを確認。テスト用の包みを1つ加えて、閉じた案内から開封画面へ直接移ることも確認。事業への条件別遷移と、一覧がパネル高を超える場合のスクロールは未確認。テスト用の包み1つがシミュレーターに残っている。
