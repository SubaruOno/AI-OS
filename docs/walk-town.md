...
- 見た目の作り直しはブランチ `art/plateless-trial` でほぼ完了。小物は台なしの絵で15種(ピクニックテーブル・ひまわりの鉢・石の井戸を追加)、地面は島1枚の絵、影は足元にアプリ側で描く。main への取り込みは、すばるが実機で見た目を確かめてから。
- 新しい小物には手紙3通と場所2つ(木かげのピクニック、井戸ばたの花)がある。住人をタップしたときのセリフは各6つ。
- 朝のおしらせ通知(`src/notifications/`、expo-notifications)を追加。10:30に、その日0歩のときだけ住人のひとことを出す。設定でオンオフ。ネイティブの部品が増えたので、TestFlightには新しいビルドが要る。「あと200歩」「届いた」の通知はバックグラウンドで歩数を読む仕組みが要るため未着手。
- おかえりの表示にきょうの歩数を出す。
- テスト用リンクの `welcome=0` で起動時の案内も飛ばせるようになった。
