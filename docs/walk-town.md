...
- 追加確認: `./scripts/check.sh`、`git diff --check`、`npx expo lint` 通過。iOSシミュレーターで更新後のホームと「今日のこと」パネルを開き、持ち帰り候補・おてつだい・手紙まで表示されることを確認。現状のセーブデータでは内容がパネルに収まるため、長い一覧でのスクロールと、包み/事業の条件別直接遷移は未確認。
