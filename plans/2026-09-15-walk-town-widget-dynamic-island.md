...
**Created:** 2026-09-15
**Status:** 進行中(2026-09-18 に Step 1〜4 完了、ビルド5を TestFlight へ提出。実機確認から再開する)
...
確認できたこと(iPhone 17 Pro Max シミュレーター、2026-09-18):

- ウィジェット拡張が `TekutekuWidget.appex` としてビルド・署名される。
- App Groups の共有コンテナができ、アプリが `stepsToNext` `giftsWaiting` `updatedAt` を書き込む。アプリ画面の「つぎの包みまで630歩」と共有領域の値が一致した。
- ウィジェットギャラリーに「てくてくまち」が出て、実データで「あと630歩 / 15:52時点」を描画する。ホーム画面にも設置できる。
- 画面: `~/Pictures/ai/20260918-1557-walktown-widget-home.png`
...
EAS ビルド5(コミット `1696d51`)は 2026-09-18 17:41 に成功し、すばるが TestFlight へ提出した。App Groups の権限でも署名が通り、9/16 にプッシュ通知の権限で落ちた件は再発しなかった。

残っていること:

- 実機でウィジェット(Small・Medium)とダイナミックアイランドを触る(Step 5)。歩数計が動く実機でしか確かめられない部分。
- あわせて台なしの小物の見た目を確認し、`art/plateless-trial` を main に取り込むか決める。
