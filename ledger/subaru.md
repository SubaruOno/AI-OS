...
- 2026-09-16 21:50 · すばる · walk-town/build · Codexに今夜の差分の不具合探しを依頼し4件を検証・対応: 新しい手紙を途中に入れたためTestFlight利用者の手紙の進行がずれ報酬を再取得できる(末尾に移動、10通目の表示を確認)、予約した持ち帰りに前日の歩数が入る(予約日の0時から数える)、日付をまたいだお礼画面で未達成の依頼の報酬を受け取れる(カードをIDで作り直し、日付を検証)。朝の通知がアプリを開かずに歩いた日にも届く件はバックグラウンド処理が要るためCONTRACTに制約として記録
- 2026-09-16 21:55 · すばる · walk-town/build · 小物を3種追加(木のブランコ・巣箱のポール・小さな本の箱、Codex画像生成、台なし)で18種に。住人のお気に入りと日替わりのお願い3種にも追加。シミュレーターで比率を確認
- 2026-09-16 21:58 · すばる · walk-town/build · 新しい場所2つ(ベンチの読書コーナー、小鳥の来る木)と、その舞う飾りを追加。iOS向けの書き出し(expo export)が通ることを確認。Codexは利用上限に達し 2026-09-17 01:59 まで使えない
- 2026-09-16 22:00 · すばる · walk-town/build · 手紙・場所・お気に入り・日替わりのお願い・絵の高さで使う小物IDがカタログに実在するかを確かめる scripts/check_item_ids.py を作り、check.sh に組み込み。わざと誤字を入れて検出を確認
