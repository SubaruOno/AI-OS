...
- 2026-09-18 15:15 · すばる · walk-town/build · 9/17の未コミット修正2件を検証してコミット。最後の包みのボタン重複(表示名と動きの判定を`listUnopenedGifts()`にそろえる)、ドラッグ中のマス目表示(置き先のひし形を運んでいる小物のzIndex 2000より上の2500へ)。型・lint・小物ID検査は通過。「他の小物を半透明」はすでに`active.value >= 0`でドラッグ中も効いており、追加の変更は不要と判明
- 2026-09-18 15:30 · すばる · walk-town/ship · art/plateless-trial(61コミット)をGitHubへpush。これまでこのMacにしか存在しなかった。mainへのマージは実機で見た目を確かめてから
- 2026-09-18 15:35 · すばる · walk-town/research · まとめ開封の重複を抽選式ごと写して20万回シミュレーション。7個開封で重複38.7〜60.1%と確率どおりで不具合ではないと結論。未所持優遇も手紙の優遇も所持数の更新を正しく追っていた。[記録](../outputs/walk-town/2026-09-18-duplicate-draws.md)
- 2026-09-18 15:40 · すばる · walk-town/note · docs/walk-town.md の現状を更新(push済み・直した2件・重複は直さない決定・開発ビルドの作り直しが要ること)。シミュレーターでの見た目の確認が未了である旨も明記
- 2026-09-18 15:10 · すばる · 就活/note · Gmailの受信箱を確認し、ファーストリテイリングの特別座談会2件(9/22 9:30・9/24 14:00、Teams)をカレンダーに追加
- 2026-09-18 16:20 · すばる · walk-town/build · 開発ビルドを作り直してシミュレーターで2件を確認。最後の包みの画面はボタン1つだけになった。小物14個の密な街を仕込んでマス目の見え方を確認し、半透明の小物越しにはっきり読めることを確認(仮説1の「小物の陰に隠れる」は起きない)。ドラッグも移動が成立しクラッシュなし
